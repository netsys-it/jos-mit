...
boot_alloc(uint32_t n)
{
	...
	if (n != 0) {
		if(n > 0x00400000 || n > (PGSIZE * npages - (uint32_t)(nextfree)))
			panic("boot_alloc: Not enough memory avaiable\n");
		result = nextfree;
...
page_free(struct PageInfo *pp)
{
	...
	if(pp->pp_link != NULL)
		panic("page_free: pp->pp->link != NULL\n");
...
pgdir_walk(pde_t *pgdir, const void *va, int create) {
	...
	size_t pdx = PDX(va);		// page dir index
	pde_t pde = pgdir[pdx];		// page dir entry
